...
const ProjectModal: React.FC<ProjectModalProps> = ({ project, lang, onClose, theme }) => {
    ...
    return (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4">
            ...
            <motion.div
                ...
                className={`relative ${isDark ? 'bg-[#1a1c23] border-blue-500/50' : 'bg-white border-blue-200'} border-2 w-full max-h-[85vh] overflow-hidden rounded-[2rem] flex flex-col lg:flex-row shadow-[0_0_100px_rgba(59,130,246,0.3)] z-[99999] ${project.hideImage ? 'max-w-2xl' : 'max-w-5xl'}`}
            ...
            >
                ...
                {!project.hideImage && (
                    <div className="lg:w-1/2 h-48 lg:h-auto overflow-hidden relative">
                        ...
                        <div className={`absolute inset-0 bg-gradient-to-t ${isDark ? 'from-[#1a1c23]' : 'from-white'} via-transparent to-transparent`} />
                    </div>
                )}

                {/* Right Side: Content */}
                <div className={`${project.hideImage ? 'w-full text-center' : 'lg:w-1/2'} p-8 sm:p-12 overflow-y-auto custom-scroll flex flex-col ${isDark ? 'bg-[#1a1c23]' : 'bg-white'}`}>
                    <div className="mb-6">
                        <span className="text-blue-500 text-[10px] font-black uppercase tracking-[0.4em] mb-3 block">{project.category}</span>
                        <h2 className={`text-3xl sm:text-4xl font-black tracking-tighter uppercase italic leading-tight ${isDark ? 'text-white' : 'text-slate-900'} mb-4`}>
                            {project.title[lang]}
                        </h2>
                    </div>

                    <div className={`${isDark ? 'text-slate-300' : 'text-slate-600'} text-sm leading-relaxed font-light mb-8 opacity-90 ${project.hideImage ? 'mx-auto max-w-lg' : ''}`}>
                    ...
                    <div className="mb-8">
                        ...
                        <div className={`flex flex-wrap gap-2 ${project.hideImage ? 'justify-center' : ''}`}>
                            {project.tech.map(tech => (
                                <span key={tech} className={`px-3 py-1 ${isDark ? 'bg-blue-600/10 border-blue-600/20' : 'bg-blue-50 border-blue-100'} border text-blue-500 text-[9px] font-black rounded-full uppercase tracking-widest`}>
                                    {tech}
                                </span>
                            ))}
                        </div>
                    ...
                    <div className={`mt-auto pt-8 border-t ${isDark ? 'border-white/5' : 'border-slate-100'} flex flex-col sm:flex-row gap-4 ${project.hideImage ? 'justify-center' : ''}`}>
                        {project.liveUrl && (
                        ...
                        <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className={`flex-1 py-4 border ${isDark ? 'border-white/20 text-white hover:bg-white/5' : 'border-slate-200 text-slate-700 hover:bg-slate-50'} rounded-2xl text-center font-black text-xs uppercase tracking-widest transition-all ${project.hideImage ? 'max-w-xs mx-auto' : ''}`}>
